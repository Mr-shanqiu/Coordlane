...
1. Run read-only `doctor`. Skill loaded is not enabled; stop on a red result
   and show the exact repair command instead of silently coordinating in chat.
2. At the Turn-entry gate, scan registered, non-archived Crew with active
...
3. Audit workspace, branch, HEAD, dirty state, authoritative truth sources,
   dependencies, ownership, runtime switches, and side effects.
4. Create an `assignment_id`, `parent_decision_id`, `scope_version`, attempt,
   origin, ownership epoch, risk tier, business goal, first-value action,
   evidence boundary, enforceable counters, and explicit branch policy before
   dispatch. Derive locks from a digest-bound authority manifest when present.
5. Treat send success as delivery only. Read the target and require
...
6. Keep shared entry points under one writer. Block running on overlap or an
   unmet dependency; unresolved truth allows read-only audit or skeleton only.
7. At safe points, let `PostToolUse(wait_threads)` attest actual target coverage
...
8. Use Captain review for bounded read-only R0; require independent Validator
   evidence for R2 and boundary-sensitive R1. Review
...
9. After validation, authorize one Dock Crew with a decision ID, exact source
...
10. Drain all terminals to a fixed high-water mark. Adjudicate every consumed
   terminal, then dispatch its required next action or explicitly defer it.
11. Release ownership and close only after every release-evidence field passes.
12. Invalidate the old final gate at the start of every user turn. Before every
...
13. Distinguish active-turn consistency from sleeping-controller liveness.
