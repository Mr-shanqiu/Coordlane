...
Current store schema version: **1.2.0**.

Opening a Coordlane store at schema `1.0.0` or `1.1.0` performs a locked,
...
and writes the project version last. New stores start at `1.2.0`. Unknown or
...
`event` records for exactly `1.2.0` before status, sweep, or Hook logic proceeds;
...
Eight JSON Schemas define Coordlane's current records:

- `authority-manifest.schema.json`: active authority identity, derived file
  locks, stop conditions, and digest binding;
- `assignment.schema.json`: identity, origin, scope, risk, business progress,
  enforceable budgets, attempt, delivery, ACK, disposition, and integration;
